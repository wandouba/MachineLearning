...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            #self.epsilon = math.pow(0.96, self.env.round)
    ...
    def learn(self, state, action, reward):
        ...
        action_qv_map = self.Q[state]
        q_value = 0
        if action_qv_map.has_key(action):
            q_value = action_qv_map[action]
        q_value = (1 - self.alpha) * q_value + self.alpha * reward
        action_qv_map[action] = q_value
...
def run():
    ...
    sim = Simulator(env, update_delay = 0.002, log_metrics = True, optimized = True)
    ...
    sim.run(tolerance = 0.03, n_test = 20)
